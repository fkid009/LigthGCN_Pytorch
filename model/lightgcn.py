...
class LightGCN(nn.Module):
    def __init__(
        ...
        user_num: int,
        item_num: int,
        L: torch.Tensor,      # normalized adjacency: D^{-1/2} A D^{-1/2}
        embed_dim: int,
        n_layer: int,
        l2_reg: float,
    ):
        ...
        self.node_num = user_num + item_num
        ...
        # node embeddings: users + items (only trainable parameters)
    ...
    def propagate(self):
        ...
        E_item : (item_num, embed_dim)
            -> final embeddings after layer combination
        """
        # E^{(0)}
        E = self.embedding.weight  # (node_num, embed_dim)
        all_embeddings = [E]
        ...
        # layer combination: uniform weights (alpha_k = 1 / (K+1))
        # all_embeddings: list of (node_num, embed_dim)
        E_all = sum(all_embeddings) / len(all_embeddings)  # (node_num, embed_dim)
    ...
    def bpr_loss(self, user_idx, pos_idx, neg_idx):
        """
        BPR loss using current LightGCN embeddings.
        ...
        # L2 regularization on embeddings (0-th layer only in 논문, but 여기선 final도 ok)
        reg_loss = (
            u_e.norm(2).pow(2)
            + pos_e.norm(2).pow(2)
            + neg_e.norm(2).pow(2)
...
def evaluator(
    ...
    """
    Evaulate LightGCN model with leave-one-out style ranking:
...
def trainer(
    ...
    """
    Training loop for LightGCN with BPR Loss
    """
